...
void Scenario1::ScenarioEnable(Platform::Object^ sender, Windows::UI::Xaml::RoutedEventArgs^ e)
{
    if (accelerometer != nullptr)
    {
		...
		///
		/// The following reading and visibility properties can be moved to a base class.
		///
		...
		auto readings = rxrt::FromEventPattern<AccelerometerReadingChangedTypedEventHandler>(
			this,
			[](Scenario1^ that, AccelerometerReadingChangedTypedEventHandler^ h)
			{
				return that->accelerometer->ReadingChanged += h;
			},
			[](Scenario1^ that, Windows::Foundation::EventRegistrationToken t)
			{
				that->accelerometer->ReadingChanged -= t;
			});

		auto visibility = rxrt::FromEventPattern<WindowVisibilityChangedEventHandler, VisibilityChangedEventArgs>(
			Window::Current,
			[](Window^ that, WindowVisibilityChangedEventHandler^ h)
			{
				return that->VisibilityChanged += h;
			},
			[](Window^ that, Windows::Foundation::EventRegistrationToken t)
			{
				that->VisibilityChanged -= t;
			});
		...
		auto subscribeReadings =
			[this, readings]()
			{
				...
				this->readingSubscription = rx::from(readings)
		...
		this->visibilitySubscription = rx::from(visibility)
			.subscribe(
